...
#define exit(returncode) longjmp(error_exit, returncode)
#endif
@z
...
if (j!=argc-1) {
  fprintf(stderr,
    "Usage:\t%s [-l] [-v] [-b hexbase] [-u hexlimit] mmofile\n"
    "\t-l          show listing\n"
    "\t-v          be verbose\n"
    "\t-b hexbase  start image at hexbase (default #8000000000000000)\n"
    "\t-u hexlimit stop image before hexlimit (default #FFFFFFFFFFFFFFFF)\n",
    argv[0]);
@.Usage: ...@>
  exit(-1);
}
...
@y
@<Sub...@>=
static void read_tet @,@,@[ARGS((void))@];
static void read_tet()
@z
...
static int yz; /* the two least significant bytes */
static tetra tet; /* |buf| bytes packed big-endianwise */
@z
