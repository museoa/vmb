...
{ FILE *out;
  ...
  if (filename==NULL)
  { win32_error(__LINE__,"Filename expected");
    return 0;
  }
  vmb_debugs(VMB_DEBUG_PROGRESS, "writing configfile %s\n",filename);
  out=fopen(filename,"w");
  if (out==NULL) 
	  {  win32_error(__LINE__,"Could not write configuration file");
	     return 0;
	  }
...
{ FILE *in;
  ...
  if(filename==NULL)
  { win32_error(__LINE__,"Argument expected");
    return 0;
  }
