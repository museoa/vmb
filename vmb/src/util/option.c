...
{ unsigned int n;
  ...
  { *option = malloc(n+1);
	if (*option == NULL)
	{ fatal_error(__LINE__,"Out of Memory for option");
	  return;
	}
  }
  *option = malloc(n+1);
...
{ FILE *out;
  ...
  if (out==NULL) 
	  {  errormsg("Could not write configuration file");
	     return 0;
	  }
...
{ int i,n;
  ...
  if (programpath==NULL) 
  { fatal_error(__LINE__,"Out of memory");
    return;
  }
  strncpy(programpath,arg,n);
  programpath[n]=0;
  i = strlen(arg+n) + 1;
  programname = malloc(i); /* name + '0' */
  if (programname==NULL) 
  { fatal_error(__LINE__,"Out of memory");
    return;
  }
  strcpy(programname,arg+n);
  defined = malloc(i); /* name + '0' */
  if (defined==NULL) 
  { fatal_error(__LINE__,"Out of memory");
    return;
  }
  strcpy(defined,programname);
#ifdef WIN32
