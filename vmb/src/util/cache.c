...
vmb_cache vmb_i_cache;
vmb_cache vmb_d_cache;
...
static vmb_cache_line *cache_load(vmb_cache *c,  int address_hi, int address_lo)
...
static vmb_cache_line *cache_preload(vmb_cache *c, int address_hi, int address_lo) 
/* load the cache line for the given address. Dont wait for completion */

{ vmb_cache_line *line = cache_lookup(c, address_hi, LINESTART(address_lo));
  if (line==NULL)
    line = cache_load(c, address_hi, LINESTART(address_lo));
  return line;
}

 void vmb_cache_preload(vmb_cache *c, unsigned int hi_address,
			      unsigned int lo_address)
/* this is the external version of the previous function, it does not return a cache line */
{ cache_preload(c, hi_address, LINESTART(lo_address));
}

static vmb_cache_line *get_valid_line(vmb_cache *c, int address_hi, int address_lo) 
/* get a valid cache line containing the requested address */
{ vmb_cache_line *line = cache_preload(c, address_hi, LINESTART(address_lo));
  if (line->content.status != STATUS_VALID)
    vmb_wait_for_valid(&line->content);
  return line;
}
...
const unsigned char *vmb_cache_read(vmb_cache *c, unsigned int address_hi, unsigned int address_lo) 
...
*/
{ vmb_cache_line *line = get_valid_line(c, address_hi, LINESTART(address_lo));
  return line->content.data+LINEOFFSET(address_lo);
}

unsigned int vmb_cache_read_int(vmb_cache *c, 
...
unsigned char *vmb_cache_write(vmb_cache *c, unsigned  int address_hi,  unsigned int address_lo) 
...
void vmb_cache_flush(vmb_cache *c)
     /* write all the dirty lines to memory */
{ int i,k;
  for (i=0;i<CACHESIZE;i++)
    for (k=0;k<WAYS;k++)
      if (c->line[i][k].content.status==STATUS_VALID && c->line[i][k].dirty)
        { vmb_store(&c->line[i][k].content);
          c->line[i][k].dirty = 0;
        }       
}


void vmb_cache_flush_line(vmb_cache *c,int address_hi,int address_lo)
...
void vmb_cache_zero_line(vmb_cache *c,int address_hi,int address_lo)
