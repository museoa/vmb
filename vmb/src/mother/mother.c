...
remove_slot (int slotnr)
{
  if (!bus_unregister (slot[slotnr].fd))
    debugi ("Shutdown of Slot %d", slotnr);
  else
...
{
  if (send_msg
      (slot[i].fd, 1, mtype, msize, mslot, mid, mtime, maddress,
       mpayload) <= 0)
  {
...
BitmapToRegion (HBITMAP hBmp)
{
  ...
  if (hBmp)
  {
    ...
    if (hMemDC)
    {
      ...
      hbm32 =
	CreateDIBSection (hMemDC, (BITMAPINFO *) & RGB32BITSBITMAPINFO,
...
#define ALLOC_UNIT	100
	  ...
	  p32 =
	    (BYTE *) bm32.bmBits + (bm32.bmHeight - 1) * bm32.bmWidthBytes;
